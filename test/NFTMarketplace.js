...
describe("NFT Marketplace", function () {
  ...
  it("Should return the new greeting once it's changed", async function () {
    ...
    const NFT721 = await ethers.getContractFactory("CyberpunksERC721");
    nft721instace = await NFT721.deploy(nft721tokenName, nft721tokenSymbol, tokenURIPrefix);
    await nft721instace.deployed();
    const NFT1155 = await ethers.getContractFactory("CyberpunksNFT1155");
  ...
  it(`Revert condition - Mint functionality - Owner sign verification failed`, async () => {
    ...
    const uri = "sample1";
    const royaltyfee = 5
    await expect(nft721instace.connect(owner).mint(uri, royaltyfee, [v, r, s, nonce_ownersignature_1])).to.be.revertedWith('Owner sign verification failed')
  })

  it(`Checking tokenId after minted`, async () => {
    let tokenId_aftermint = 1
  ...
  it(`Executing bid Revert - Checking : 'Nonce : Invalid Nonce'`, async () => {
    ...
    let amount = 1025
    let qty = 1
    await expect(tradeinstance.connect(user2).executeBid([seller, buyer, erc20Address, nftAddress, nftType, unitPrice, amount, tokenId, qty], [v_buyer_exec, r_buyer_exec, s_buyer_exec, nonce_sellersignature])).to.be.revertedWith("Nonce : Invalid Nonce")
  })
  ...
  it(`Executing bid Revert - Checking : 'buyer sign verification failed'`, async () => {
    ...
    let amount = 1025
    let qty = 1
    await expect(tradeinstance.connect(user2).executeBid([seller, buyer, erc20Address, nftAddress, nftType, unitPrice, amount, tokenId, qty], [v_buyer_exec, r_buyer_exec, s_buyer_exec, temp_nonce])).to.be.revertedWith("buyer sign verification failed")
  })
  ...
  it(`Revert condition - Mint functionality - Owner sign verification failed`, async () => {
    ...
    const royaltyfee = 5
    const supply = 1
    await expect(nft1155instance.connect(owner).mint(uri, supply, royaltyfee, [v, r, s, nonce_ownersignature_1])).to.be.revertedWith('Owner sign verification failed')
  })
